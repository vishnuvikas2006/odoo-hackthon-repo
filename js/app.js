...
async function renderDashboardKPIs() {
  ...
  const driverPortal = document.getElementById("driver-dashboard-portal");
  const defaultFilters = document.getElementById("dashboard-default-filters");
  const kpiGrid = document.querySelector("#panel-dashboard .dashboard-grid");
  const chartsGrid = document.querySelector("#panel-dashboard .charts-grid");

  if (currentUser && currentUser.role === "Driver") {
    if (driverPortal) driverPortal.style.display = "block";
    if (defaultFilters) defaultFilters.style.display = "none";
    if (kpiGrid) kpiGrid.style.display = "none";
    if (chartsGrid) chartsGrid.style.display = "none";

    const driverSelect = document.getElementById("driver-portal-select");
    if (driverSelect && driverSelect.options.length === 0) {
      ...
      if (currentUser.driverId) {
        driverSelect.value = currentUser.driverId;
      }
    }

    const verifyBtn = document.getElementById("btn-driver-portal-verify");
    if (verifyBtn && !verifyBtn.getAttribute("data-bound")) {
      ...
      verifyBtn.addEventListener("click", async () => {
        ...
        const enteredLicense = document.getElementById("driver-portal-license").value.trim().toUpperCase();

        if (!enteredLicense) {
          window.showToast("Please enter your Driving License number.", "warning");
          return;
        }

        const driversList = await db.getDrivers();
        const matched = driversList.find(d => d.id === selectedId);

        if (matched && matched.licenseNumber.replace(/\s+/g, "").toUpperCase() === enteredLicense.replace(/\s+/g, "")) {
          document.getElementById("driver-portal-authenticated").style.display = "block";
          document.getElementById("driver-portal-img").src = matched.photo || "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&h=150&fit=crop";
          document.getElementById("driver-portal-name").textContent = matched.name;
          
          const statusBadge = document.getElementById("driver-portal-verify-status");
          statusBadge.textContent = matched.verificationStatus;
          statusBadge.className = `badge badge-${matched.verificationStatus.toLowerCase()}`;

          document.getElementById("driver-portal-license-cat").textContent = matched.licenseCategory;
          document.getElementById("driver-portal-license-exp").textContent = matched.licenseExpiry;
          document.getElementById("driver-portal-contact").textContent = matched.contactNumber || 'N/A';
          document.getElementById("driver-portal-emergency").textContent = matched.emergencyContact;
          document.getElementById("driver-portal-experience").textContent = `${matched.experience} Years`;
          document.getElementById("driver-portal-safety-score").textContent = `${matched.safetyScore}/100`;

          const tripsList = await db.getTrips();
          const vehiclesList = await db.getVehicles();
          const driverTrips = tripsList.filter(t => t.driverId === matched.id);

          const completedTrips = driverTrips.filter(t => t.status === "Completed");
          const totalDist = completedTrips.reduce((sum, t) => sum + parseFloat(t.plannedDistance || 0), 0);
          const totalEarnings = completedTrips.reduce((sum, t) => sum + (parseFloat(t.plannedDistance || 0) * 30), 0);

          document.getElementById("driver-portal-completed-count").textContent = completedTrips.length;
          document.getElementById("driver-portal-total-dist").textContent = `${totalDist.toLocaleString()} km`;
          document.getElementById("driver-portal-total-earnings").textContent = `${totalEarnings.toLocaleString()} INR`;

          const tripsBody = document.getElementById("driver-portal-trips-body");
          tripsBody.innerHTML = "";

          if (driverTrips.length === 0) {
            tripsBody.innerHTML = `<tr><td colspan="7" style="text-align:center;">No shifts assigned to this profile.</td></tr>`;
          } else {
            driverTrips.forEach(t => {
              const v = vehiclesList.find(veh => veh.id === t.vehicleId);
              const tr = document.createElement("tr");
              tr.innerHTML = `
                <td><b>${t.id}</b></td>
                <td>${t.source} &rarr; <br>${t.destination}</td>
                <td>${v ? v.registrationNumber : 'Unknown'}</td>
                <td>${t.cargoWeight} kg</td>
                <td>${t.plannedDistance} km</td>
                <td>${new Date(t.deliverySchedule).toLocaleString()}</td>
          ...
          lucide.createIcons();
          window.showToast("Driver profile authenticated and verified successfully!", "success");
        } else {
          document.getElementById("driver-portal-authenticated").style.display = "none";
          window.showToast("Authentication Failed: Driving License number does not match.", "danger");
        }
      });
    }
    return;
  ...
  if (driverPortal) driverPortal.style.display = "none";
  if (defaultFilters) defaultFilters.style.display = "flex";
...
async function renderVehiclesTable() {
  ...
  vehicles.forEach(v => {
    ...
    const matchesSearch = v.registrationNumber.toLowerCase().includes(searchVal) || vModel.toLowerCase().includes(searchVal);
    const matchesStatus = filterStatus === "All" || vStatus === filterStatus;
    ...
    row.innerHTML = `
      ...
      <td><span class="badge badge-${vStatus.toLowerCase().replace(" ", "")}">${vStatus}</span></td>
  ...
  bodyEl.innerHTML = `
    <div style="display:grid; grid-template-columns:1fr; gap:16px; font-family:var(--font-body); font-size:13px;">
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px; background:var(--bg-tertiary); padding:16px; border-radius:10px;">
        <div><b>Model:</b> ${v.model || "Unknown"}</div>
        ...
        <div><b>Current Status:</b> <span class="badge badge-${(v.status || "Available").toLowerCase().replace(" ", "")}">${v.status || "Available"}</span></div>
      ...
      <div>
        <h4 style="margin-bottom:8px; display:flex; align-items:center; gap:6px;"><i data-lucide="folder-open"></i> Supporting Documents</h4>
        ${(() => {
          let docsHtml = '<p style="color:var(--text-muted); font-size:11px;">No documents uploaded.</p>';
          if (v.supportingDocs && Object.keys(v.supportingDocs).length > 0) {
            docsHtml = '<ul style="list-style:none; padding:0; display:flex; flex-direction:column; gap:6px;">';
            Object.entries(v.supportingDocs).forEach(([filename, base64Data]) => {
              const isDataUri = String(base64Data).startsWith("data:");
              const href = isDataUri ? base64Data : `data:application/pdf;base64,${base64Data}`;
              docsHtml += `
                <li style="display:flex; justify-content:space-between; align-items:center; padding:6px 12px; background:var(--bg-tertiary); border-radius:6px;">
                  <span style="font-size:11px; display:flex; align-items:center; gap:6px; max-width:180px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"><i data-lucide="file-text" style="width:14px; height:14px; color:var(--primary);"></i> ${filename}</span>
                  <a href="${href}" download="${filename}" class="btn-primary" style="padding:2px 8px; font-size:10px; text-decoration:none; height:22px; display:inline-flex; align-items:center; gap:4px; font-weight:bold;"><i data-lucide="download" style="width:10px; height:10px;"></i> Download</a>
                </li>
              `;
            });
            docsHtml += '</ul>';
...
window.editVehicle = async function(vehicleId) {
  ...
  const fileInput = document.getElementById("vehicle-file-upload");
  const fileData = document.getElementById("vehicle-uploaded-file-data");
  const fileName = document.getElementById("vehicle-uploaded-file-name");
...
function initFormListeners() {
  ...
  document.getElementById("vehicle-file-upload")?.addEventListener("change", (e) => {
    const file = e.target.files[0];
    if (!file) return;

    const reader = new FileReader();
    reader.onload = function(evt) {
      document.getElementById("vehicle-uploaded-file-data").value = evt.target.result;
      document.getElementById("vehicle-uploaded-file-name").value = file.name;
      window.showToast(`Selected file: ${file.name} for upload`, "success");
    };
  ...
  document.getElementById("vehicle-form")?.addEventListener("submit", async (e) => {
    ...
    const uploadedData = document.getElementById("vehicle-uploaded-file-data")?.value;
    const uploadedName = document.getElementById("vehicle-uploaded-file-name")?.value;
    ...
    if (id) {
      vehicle.id = id;
      const existing = await db.getVehicleById(id);
      if (existing && existing.supportingDocs) {
        vehicle.supportingDocs = existing.supportingDocs;
      }
    }

    if (uploadedData && uploadedName) {
      if (!vehicle.supportingDocs) vehicle.supportingDocs = {};
      vehicle.supportingDocs[uploadedName] = uploadedData;
    }
